...
function init() {
    ...
    // 环境光 - 冷调蓝紫微光，模拟阴冷教学楼
    const ambientLight = new THREE.AmbientLight(0x2a2a40, 0.35);
    scene.add(ambientLight);
    
    // 月光方向光 - 冷蓝色月光
    const moonLight = new THREE.DirectionalLight(0x4466aa, 0.2);
...
function createClassroomsAndCorridors(tileSize, gridWidth, gridHeight, wallHeight, material) {
    ...
    for (let x = -halfW + 15; x < halfW; x += 18) {
        ...
        scene.add(tube);
        // 走廊白炽灯 - 冷白光，闪烁
        const hallPointLight = new THREE.PointLight(0xccddff, 1500, 18, 1.3);
...
function createLights() {
    ...
    for (let i = 0; i < lightPositions.length; i++) {
        ...
        const light = new THREE.PointLight(0xffcc88, 3.5, 20, 1.5);
    ...
    for (let i = 0; i < flickerLightDefs.length; i++) {
        ...
        // 灯光
        const flickerLight = new THREE.PointLight(0xffeedd, 3.0, 18, 1.5);
...
function toggleHide() {
    if (gameState.isHiding) {
        ...
        if (gameState.flashlightLight) {
            gameState.flashlightLight.intensity = gameState.flashlightOn && gameState.battery > 0 ? 3000 : 0;
        }
...
function updateFogByCamera() {
    ...
    // far = near + 渐变区长度
    scene.fog.far = scene.fog.near + 35;
}
