...
function init() {
    ...
    // 月光方向光 - 冷蓝色月光（提亮）
    const moonLight = new THREE.DirectionalLight(0x5577bb, 0.35);
...
function createClassroomsAndCorridors(tileSize, gridWidth, gridHeight, wallHeight, material) {
    ...
    for (let x = -halfW + 15; x < halfW; x += 18) {
        ...
        scene.add(tube);
        // 走廊白炽灯 - 冷白光，闪烁
        const hallPointLight = new THREE.PointLight(0xccddff, 2200, 22, 1.3);
...
function createFlashlight() {
    ...
    const flashlightLight = new THREE.SpotLight(0xddeeff, 4500, 30, Math.PI / 6, 0.7, 3);
    ...
    const playerGlow = new THREE.PointLight(0xccddff, 25, 5, 2);
